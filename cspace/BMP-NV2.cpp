...
int main(){
    ...
    if( (fbmp=fopen("ElCapitain.bmp","rb")) == NULL ){
        puts("Fail to open file bmp!");
        exit(0);
    }
    if( (frgb=fopen("ElCapitainRgb.txt","wb+")) == NULL ){
       puts("Fail to open file Rgb!");
       exit(0);
   }
    if( (fyuv=fopen("ElCapitainYuv.txt","wb+")) == NULL ){
	...
	int span,high=bih.biHeight;//span每行多少字节 
    /*
        offset文件指针位置调节+
        别的不同位数的bmp处理
    
    */
    // span = bih.biWidth+ (4-bih.biWidth%4);//填充字节后跨度
    /*
        奇数宽度处理 舍弃最后一个
    */
    ...
    for(i=high-1;i>=0;i--){
        ...
        // char c[4];
        // fread(c,sizeof(pb)*(4-bih.biWidth%4),1,fbmp);
    }
	...
	for(i=0;i<high;i++){
        for(j=0;j<bih.biWidth;j++){
            ...
            //写nv21
            fwrite(&yuvImage[i*bih.biWidth+j].y,sizeof(yuvImage[i*bih.biWidth+j].y),1,fnv21);
        }
    }
    //奇数，则舍去最后一列
    int wid=(bih.biWidth%2==1)?bih.biWidth-1:bih.biWidth;
    //写nv21

    for(i=0;i<high;i++){
        for(j=0;j<wid;j++){
            if(0==i%2&&0==j%2){
                fwrite(&(yuvImage[i*bih.biWidth+j].v),sizeof(yuvImage[i*bih.biWidth+j].v),1,fnv21);
            }else if(1==i%2&&1==j%2){
                fwrite(&(yuvImage[i*bih.biWidth+j].u),sizeof(yuvImage[i*bih.biWidth+j].u),1,fnv21);
            }
        }
    }
